...
export class HomeComponent implements OnInit {
  ...
  submit() {
    ...
    this.submitted = true;
    this._api
      .getReaction(`reaction/get_reaction?minute=${0}&cnt_id=${this.cnt_id}`)
      .subscribe((data: any) => {
        if (data && !data.error) {
    ...
    this._api
      .getDetails(`content/get_details?cnt_id=${this.cnt_id}`)
      .subscribe((data: any) => {
        if (data && !data.error) {
  ...
  submitEmotion() {
    this.getEmotion(
      this.cnt_id,
      this.cohort,
      this.emotion_form.value.time_range
    );
  ...
  Progress(e: any) {
    ...
    this.data =
      this.emotionDataAll.graph_data[
  ...
  Start(e: any) {
    ...
    this.interval2 = setInterval(() => {
      ...
      if (!this.Waiting) {
        ...
        for (let i = 0; i < this.emotionDataAll.length; i++) {
          if (
            this.emotionDataAll[i].time === Math.floor(this.videoCurrentTime)
          ) {
    ...
    this.interval = setInterval(() => {
      this.Duration = Math.ceil(
        (this.videoPlay.nativeElement.currentTime /
          this.videoPlay.nativeElement.duration) *
          100
      );
    }, 5);
  }
  ...
  getCohortsValue(cnt_id: any) {
    this._api
      .getCohorts(`additional_cohorts/get_additional_cohorts?cnt_id=${cnt_id}`)
  ...
  selectCohort(index: any, value: any) {
    this.cohort = {};
    index === 0
      ? (this.cohort['emoting'] = value)
      : index === 1
      ? (this.cohort['age_range'] = value)
      : index === 2
      ? (this.cohort['gender'] = value)
      : (this.cohort['slag'] = value);
  }
  ...
  getEmotion(cnt_id: any, cohort: any, time_range: any) {
    ...
    if (Object.keys(cohort).length > 0) {
      this._api
        .getEmotion(
          `emotion/get_emotion?minute=${0}&cnt_id=${cnt_id}&${
            Object.keys(cohort)[0]
          }=${Object.values(cohort)[0]}`
        )
        .subscribe((res: any) => {
          if (res && !res.error && res.response.graph_data) {
            ...
            if (this.reactionData) {
              this.emotionDataAll = this.emotionDataAll.map(
                (item: any, index: any) => {
                  return {
                    ...item,
                    ...(this.reactionData[index]
                      ? this.reactionData[index]
                      : {}),
                  };
                }
              ...
              if (
                ...
                this.start >= 0 &&
                this.end > this.start &&
                this.start < this.emotionDataAll.length &&
                this.end <= this.emotionDataAll.length
              ) {
                this.customTime = true;
                this.emotionDataAll = this.emotionDataAll.filter(
                  (item: any) => {
      ...
      this._api
        .getEmotion(`emotion/get_emotion?minute=${0}&cnt_id=${cnt_id}`)
        .subscribe((res: any) => {
          if (res && !res.error && res.response.graph_data) {
            this.emotionDataAll = res.response.graph_data;
            this.showSelect = true;
